<template>
  <q-page class="flex flex-center">
    <div class="q-pa-md q-gutter-sm">
      ...
      <q-btn label="MY BUDGETS" color="primary" @click="bar2 = true" disabled />
...
export default defineComponent({
  ...
  setup() {
    ...
    const { getServiceHistoryByID, getServiceList_by_username } = useApi();

    const mapServices_by_username = async (value) => {
      console.log(value);
      try {
        serviceList.value = await getServiceList_by_username(value);
      } catch (error) {
        notifyError(error);
      }
    };
